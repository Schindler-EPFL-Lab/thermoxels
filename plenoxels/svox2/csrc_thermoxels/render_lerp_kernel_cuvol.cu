...
__device__ __inline__ void trace_ray_cuvol_rgbt(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t lane_id,
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            float lane_temp = trilerp_cuvol_one(
                grid.links,
            ...
            t_outv += weight * fmaxf(lane_temp,0.0f); //
    ...
    if (grid.background_nlayers == 0) {
        rgb_outv += _EXP(log_transmit) * opt.background_brightness;
    }
    if (lane_colorgrp_id == 0) {
        if (out_log_transmit != nullptr) {
            *out_log_transmit = log_transmit;
...
__device__ __inline__ void trace_ray_surface_temperature(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        float* __restrict__ out) {
...
#pragma unroll 3
        ...
        float temperature = trilerp_cuvol_one(
                grid.links, grid.temperature_data,
                grid.stride_x,
                grid.size[2],
                1,
                ray.l, ray.pos,
...
__device__ __inline__ void trace_ray_temperature_surface_sigma_thresh(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        float sigma_thresh,
...
__device__ __inline__ void trace_ray_cuvol_rgbt_backward(
        const PackedSparseGridSpec& __restrict__ grid,
    ...
    float accum = fmaf(rgb_cache[0], grad_out_rgb[0],
                      fmaf(rgb_cache[1], grad_out_rgb[1],
                           rgb_cache[2] * grad_out_rgb[2])) + t_loss * t_cache *
                           grad_out_temp;
...
#pragma unroll 3
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            float temperature = lane_temp;
            const float lane_temperature_total =  temperature * grad_out_temp;
            float total_temperature = fmaxf(lane_temperature_total, 0.f);
            ...
            const float curr_grad_temperature = weight * grad_out_temp;
            // With or without lane_temp
//              w_i*dL/dt_i
            if (grid.basis_type != BASIS_TYPE_SH) {
                float curr_grad_sphfunc = lane_color * grad_common;
                const float curr_grad_up2 = __shfl_down_sync((1U << grid.sh_data_dim) - 1,
            ...
            accum -= weight * (total_color+ t_loss*total_temperature);
            float curr_grad_sigma = ray.world_step * (
                    (total_color+ t_loss*total_temperature) * _EXP(log_transmit) -
                    accum);
                //             ray.world_step is sigma_i
                //             _EXP(log_transmit) is Ti
                // (total_color + t_loss*total_temperature) is (Ci - ti)
                //  accum is sum (c_j +t_j)* w_j
            ...
            if (lane_id == 0) {
                trilerp_backward_cuvol_one_temperature(
                        grid.links,
                        grads.grad_temperature_out,
                        grid.stride_x,
                        grid.size[2],
                        ray.l, ray.pos,
                        curr_grad_temperature);
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_kernel_rgbt(
        PackedSparseGridSpec grid,
        PackedRaysSpec rays,
        RenderOptions opt,
    ...
    trace_ray_cuvol_rgbt(
        ...
        temp_storage[ray_blk_id],
        rgb_out[ray_id].data(),
        temp_out+ ray_id, // MIGHT BE THE ISSUE
//         tempr_out[ray_id].data(),
        log_transmit_out == nullptr ? nullptr : log_transmit_out + ray_id);
}
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_temperature_surface_sigma_thresh_kernel(
        PackedSparseGridSpec grid,
        PackedRaysSpec rays,
        RenderOptions opt,
        float sigma_thresh,
    ...
    trace_ray_temperature_surface_sigma_thresh(
        grid,
        ray_spec,
        opt,
        sigma_thresh,
...
__launch_bounds__(TRACE_RAY_BKWD_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_backward_kernel_rgbt(
    PackedSparseGridSpec grid,
    const float* __restrict__ rgb_gt,
    const float* __restrict__ temp_gt,
    const float* __restrict__ rgb_cache,
    const float* __restrict__ temperature_cache,
    const float* __restrict__ custom_temperature_gradient,
    float* __restrict__ temperature_surface,
    PackedRaysSpec rays,
    RenderOptions opt,
    bool grad_out_is_rgb,
    const float* __restrict__ log_transmit_in,
    ...
    // Add L1 loss derivative to temp_grad_out
    const float l1_derivative = (temperature_cache[ray_id] > temp_gt[ray_id]) -
                                (temperature_cache[ray_id] < temp_gt[ray_id]);

    const float t_resid = l2_loss*(temperature_cache[ray_id] - temp_gt[ray_id]) +
                        surface_loss*(temperature_cache[ray_id] -
                        temperature_surface[ray_id]) + l1_loss * l1_derivative;
...
#pragma unroll 3
        for (int i = 0; i < 3; ++i) {
            const float resid = rgb_cache[ray_id * 3 + i] - rgb_gt[ray_id * 3 + i];
            rgb_grad_out[i] = resid * norm_factor;
        }
        temp_grad_out = custom_temperature_gradient[ray_id] +
                        3.f * norm_factor *  t_resid; // 1
//          channel in thermal images
...
#pragma unroll 3
    ...
    __syncwarp((1U << grid.sh_data_dim) - 1);
    trace_ray_cuvol_rgbt_backward(
        grid,
        rgb_grad_out,
        temp_grad_out,
        rgb_cache + ray_id * 3,
//         temperature_cache + ray_id, // TO CHECK
        temperature_cache[ray_id],
        ray_spec[ray_blk_id],
        opt,
        lane_id,
        sphfunc_val[ray_blk_id],
...
__launch_bounds__(TRACE_RAY_CUDA_THREADS, MIN_BLOCKS_PER_SM)
__global__ void render_ray_surface_temperature_kernel(
        PackedSparseGridSpec grid,
        PackedRaysSpec rays,
        RenderOptions opt,
        float* __restrict__ out) {
...
torch::Tensor volume_render_surface_temperature(SparseGridSpec& grid,
        RaysSpec& rays, RenderOptions& opt) {
    auto options =
        torch::TensorOptions()
        .dtype(rays.origins.dtype())
    ...
    const auto Q = rays.origins.size(0);
    const int blocks = CUDA_N_BLOCKS_NEEDED(Q, TRACE_RAY_CUDA_THREADS);
    device::render_ray_surface_temperature_kernel<<<blocks, TRACE_RAY_CUDA_THREADS>>>(
            grid,
            rays,
            opt,
            results.data_ptr<float>()
...
Tensor render_pos_forward_rgbt(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        torch::Tensor rgb_out,
    ...
    {
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q * WARP_SIZE, TRACE_RAY_CUDA_THREADS);
        device::render_ray_kernel_rgbt<<<blocks, TRACE_RAY_CUDA_THREADS>>>(
                    grid, rays, opt,
                    // Output
                    rgb_out.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                    temp_out.data_ptr<float>(),
                    log_transmit.data_ptr<float>());
    }
    if (use_background) {
        // printf("RENDER BG\n");
...
torch::Tensor volume_render_surface_temperature_sigma_thresh(
SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        float sigma_thresh) {
    ...
    const auto Q = rays.origins.size(0);
    const int blocks = CUDA_N_BLOCKS_NEEDED(Q, TRACE_RAY_CUDA_THREADS);
    device::render_ray_temperature_surface_sigma_thresh_kernel<<<blocks, TRACE_RAY_CUDA_THREADS>>>(
            grid,
            rays,
            opt,
            sigma_thresh,
...
void volume_render_cuvol_rgbt_fused(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        torch::Tensor rgb_gt,
        torch::Tensor temp_gt,
        torch::Tensor custom_temperature_gradient,
        float l2_loss,
        float beta_loss,
        float sparsity_loss,
        float t_sigma_loss,
    ...
    CHECK_INPUT(temp_gt);
    CHECK_INPUT(temp_out);
    CHECK_INPUT(custom_temperature_gradient);
    grid.check();
    rays.check();
    grads.check();
    const auto Q = rays.origins.size(0);
    ...
    torch::Tensor temperature_surface = torch::empty({rays.origins.size(0)}, options);
    if (sigma_thresh > 0.f) {
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q, TRACE_RAY_CUDA_THREADS);
        device::render_ray_temperature_surface_sigma_thresh_kernel<<<blocks, TRACE_RAY_CUDA_THREADS>>>(
                grid,
                ...
                opt,
                sigma_thresh,
                temperature_surface.data_ptr<float>());
    }
    else {
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q, TRACE_RAY_CUDA_THREADS);
        device::render_ray_surface_temperature_kernel<<<blocks, TRACE_RAY_CUDA_THREADS>>>(
    ...
    {
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q * WARP_SIZE, TRACE_RAY_BKWD_CUDA_THREADS);
        device::render_ray_backward_kernel_rgbt<<<blocks, TRACE_RAY_BKWD_CUDA_THREADS>>>(
                grid,
                rgb_gt.data_ptr<float>(),
                temp_gt.data_ptr<float>(),
                rgb_out.data_ptr<float>(),
                temp_out.data_ptr<float>(),
                custom_temperature_gradient.data_ptr<float>(),
                temperature_surface.data_ptr<float>(),
                rays, opt,
                true,
                beta_loss > 0.f ? log_transmit.data_ptr<float>() : nullptr,
...
void volume_render_cuvol_rgbt_forward(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        //output
    ...
    bool use_background = grid.background_links.defined() &&
                          grid.background_links.size(0) > 0;
    bool need_log_transmit = use_background ;
...
//     log_transmit = need_log_transmit ? _get_empty_1d(rays.origins) : torch::Tensor();
    torch::Tensor accum;
...
void volume_render_cuvol_rgbt_backward(
        SparseGridSpec& grid,
        RaysSpec& rays,
        RenderOptions& opt,
        torch::Tensor rgb_gt,
