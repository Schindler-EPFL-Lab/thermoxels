...
class EvalParameters:
    ...
    experiment_name: str = "plenoxel-autotune"
...
def main() -> None:
    ...
    data_asset = ml_client.data.get("exhibition_building_plenoxel", version="1")

    job_name = "autotune-plenoxel--" + datetime.now().strftime("%d-%m-%Y-%H%M%S")
    ...
    job = command(
        ...
        compute="voxel-a100",
        command=(
            "python3.10 plenoxels/opt/find_best_config.py "
            "--data_root ${{inputs.data}} "
            "--train_root ./ "
            "-g '0' plenoxels/opt/tasks/multi_config.json "
        ),
